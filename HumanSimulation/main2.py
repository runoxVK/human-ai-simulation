...
class Human:
    ...
    def breed(self, humans):
        ...
        if self.target_mate:
            ...
            if math.hypot(self.x - target_x, self.y - target_y) < 10:
                # Create a new human at a random nearby position with incremented generation
                ...
                self.last_breed_time = pygame.time.get_ticks()  # Set last breed time
                self.target_mate = None
    ...
    def draw_stats(self, surface):
        ...
        text_y_offset = 20
        total_text_height = 7 * text_y_offset
        ...
        surface.blit(generation_text, (self.x - generation_text.get_width() // 2, start_y + 4 * text_y_offset))

        if self.sleeping:
            sleep_text = font.render("Sleeping", True, GRAY)
            surface.blit(sleep_text, (self.x - sleep_text.get_width() // 2, start_y + 5 * text_y_offset))
...
class Animal:
    def __init__(self, x, y):
        ...
        self.speed = 0.4  # Slightly slower than humans
...
puddles = [(random.randint(0, WIDTH), random.randint(0, HEIGHT), 40, 20) for _ in range(5)]
